const RECORD_BASE_URL = "https://kodemia-js-33g-default-rtdb.firebaseio.com/koders";

const createPet = async (petObject) => {
  let response = await fetch(`${RECORD_BASE_URL}/.json`, {
    method: "POST",
...
const fetchPetByKey = async (petKey) => {
  let response = await fetch(`${RECORD_BASE_URL}/${petKey}/.json`);
  let data = await response.json();
  return data;
};
